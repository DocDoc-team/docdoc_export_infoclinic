...
Она используется для выбора метода загрузки файлов на сервер DocDoc. И имеет 2 значения ftp или webdav
...
###### Режим сервера FireBird

В процессе экспорта в консоли может выводиться ошибка вида:

```
Statement failed, SQLSTATE = 08001
I/O error during "CreateFile (open)" operation for file "E:/Infoclinica_DB/099.fdb"
-Error while trying to open file
-╧ЁюЎхёё эх ьюцхЄ яюыєўшЄ№ фюёЄєя ъ Їрщыє, Єръ ъръ ¤ЄюЄ Їрщы чрэ Є фЁєушь яЁюЎхёёюь.
After line 0 in file .\sql\docdoc_export_clinics.sql
Use CONNECT or CREATE DATABASE to specify a database
Use CONNECT or CREATE DATABASE to specify a database
Use CONNECT or CREATE DATABASE to specify a database
```
Текст в неверной кодировке гласит: `Процесс не может получить доступ к файлу, так как этот файл занят другим процессом.`

Это говорит о том, что сервер FireBird работает в режиме Super, когда к БД разрешено только одно активное соединение.
...
После, пройти все шаги настройки заново.
